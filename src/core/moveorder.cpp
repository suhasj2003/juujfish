...
namespace Juujfish {
    ...
    void MoveOrderer::score() {
        static_assert(Gt == CAPTURES || Gt == QUIETS || Gt == EVASIONS, "Error: Invalid GenType in score().");

        Color us = pos.get_side_to_move();

        BitBoard threatened_by_pawn = pos.attacks_by<PAWN>(~us);
        BitBoard threatened_by_minor = pos.attacks_by<BISHOP>(~us) | pos.attacks_by<KNIGHT>(~us) | threatened_by_pawn;
        BitBoard threatened_by_rook = pos.attacks_by<ROOK>(~us) | threatened_by_minor;

        BitBoard threatened_pieces = ((pos.pieces(us, QUEEN) & threatened_by_rook) |
                                      (pos.pieces(us, ROOK) & threatened_by_minor) |
                                     ((pos.pieces(us, BISHOP) | pos.pieces(us, KNIGHT)) & threatened_by_pawn));
        ...
        for (auto& m : *this) {
            ...
            if (Gt == CAPTURES) {
                ...
                if (mt == ENPASSANT) {
                    ...
                    m.value = mvv_lva(promo_type, PAWN) + capture_piece != NO_PIECE ? PieceValue[type_of(capture_piece)] : 0;
                } else if (mt == CASTLING) {
                ...
                if (pos.is_capture(m)) 
                    ...
                    m.value = killers->lookup(m, ply) + (history->lookup(m, us, pt) / butterfly->lookup(m, us));
    ...
    Move MoveOrderer::next() {
        ...
        switch (stage) {
            ...
            case CAPTURE:
                if (select( [&]() { return curr->value >= 0 ? true : (*end_bad_captures++ = *curr, false); } ))
                    return *(curr - 1);
            ...
            case QUIETS_GEN:
                if (!skip_quiets) {
                    ...
                    end_moves   = begin_bad_quiets = end_bad_quiets = generate<QUIETS>(pos, curr);
