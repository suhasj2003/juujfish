...
inline BitBoard operator&(BitBoard b, Square s) {
  return BitBoard(b & square_to_bb(s));
}
inline BitBoard operator|(BitBoard b, Square s) {
  return BitBoard(b | square_to_bb(s));
}
...
inline BitBoard operator&(Square s, BitBoard b) {
  return BitBoard(square_to_bb(s) & b);
}
inline BitBoard operator|(Square s, BitBoard b) {
  return BitBoard(square_to_bb(s) | b);
}
...
constexpr BitBoard file_bb(Square s) {
  return FILE_A_BB << file_of(s);
}
constexpr BitBoard rank_bb(Rank r) {
  return RANK_1_BB << (r * 8);
}
constexpr BitBoard rank_bb(Square s) {
  return RANK_1_BB << (rank_of(s) * 8);
}

inline int manhattan_distance(Square s1, Square s2) {
  return abs(file_of(s1) - file_of(s2)) + abs(rank_of(s1) - rank_of(s2));
}
...
struct Magic {
  ...
  unsigned index(BitBoard occupied) const {
    return ((occupied & mask) * magic) >> (64 - shift);
  }
