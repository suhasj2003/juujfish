...
namespace Juujfish {
    ...
    Value Search::Worker::search(Position &pos, Value alpha, Value beta, uint8_t depth) {
        ...
        auto [table_hit, table_data, table_writer] = tt->probe(pos.get_key(), pos.generate_secondary_key());
        ...
        if (table_hit && table_data.depth >= depth && (table_data.score >= beta || table_data.score <= alpha)) {            
            prunes++;
            tt_prunes++;
        ...
        while (!m.is_nullmove()) {
            if (pos.legal(m)) {
                ...
                if (score >= beta) {
                    table_writer.write(pos.get_key(), pos.generate_secondary_key(),  depth, BOUND_LOWER, score, m );
        ...
        if (!moves_exist) {
            ...
            if (best_score <= alpha)
                table_writer.write(pos.get_key(), pos.generate_secondary_key(), depth, BOUND_UPPER, best_score, best_move );
            else
                table_writer.write(pos.get_key(), pos.generate_secondary_key(), depth, BOUND_EXACT, best_score, best_move );
        }
