...
GradedMove* generate_pawn_moves(const Position& pos, GradedMove* move_list,
                                BitBoard target) {
  ...
  if constexpr (Gt != CAPTURES) {
    ...
    BitBoard push_2 = shift<UP>(push_1 & (C == WHITE ? RANK_3_BB : RANK_6_BB)) &
                      empty_squares;
  ...
  if (pawns_on_7 != 0) {
    ...
    while (right_attack)
      move_list = generate_promotions<Gt, UP_RIGHT, true>(
          move_list, lsb(pop_lsb(right_attack)));

    while (left_attack)
      move_list = generate_promotions<Gt, UP_LEFT, true>(
          move_list, lsb(pop_lsb(left_attack)));

    while (up_push)
      move_list =
          generate_promotions<Gt, UP, false>(move_list, lsb(pop_lsb(up_push)));
  }
...
GradedMove* generate_piece_moves(const Position& pos, GradedMove* move_list,
                                 BitBoard target) {
  static_assert(
      Pt != PAWN && Pt != KING,
      "Error: Pawn and King moves are not supported by generate_moves.");
...
GradedMove* generate_moves(const Position& pos, GradedMove* move_list) {
  ...
  if (popcount(pos.get_checkers()) < 2) {
    ...
    if constexpr (Gt == EVASIONS) {
      ...
      else if (checker_pt == BISHOP || checker_pt == ROOK ||
               checker_pt == QUEEN)
        target = attacks_bb(king_sq, QUEEN, square_to_bb(checker_sq)) &
                 get_ray(king_sq, checker_sq);
      else {
        std::cerr << "Error: Unknown checker piece type in evasion generation."
                  << std::endl;
        return nullptr;
      }
  ...
  BitBoard king_moves_bb =
      attacks_bb(king_sq, KING) & (Gt == EVASIONS ? ~pos.pieces(Us) : target);
  while (king_moves_bb)
...
GradedMove* generate(const Position& pos, GradedMove* move_list) {
  static_assert(Gt != LEGAL,
                "Error: Legal moves generation is not supported right now.");
  assert((Gt == EVASIONS) == bool(pos.get_checkers()));

  Color us = pos.get_side_to_move();

  return us == WHITE ? generate_moves<WHITE, Gt>(pos, move_list)
                     : generate_moves<BLACK, Gt>(pos, move_list);
}
...
GradedMove* generate<LEGAL>(const Position& pos, GradedMove* move_list) {
  ...
  move_list = pos.is_in_check() ? generate<EVASIONS>(pos, move_list)
                                : generate<NON_EVASIONS>(pos, move_list);

  while (curr != move_list)
    if (((curr->from_sq() & pinned) || (curr->from_sq() == king_sq) ||
         (curr->type_of() == ENPASSANT)) &&
        !pos.legal(*curr))
      *curr = *(--move_list);
    else
      curr++;
