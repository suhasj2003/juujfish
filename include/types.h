...
constexpr bool is_square(Square s) {
  return s >= SQ_A1 && s < SQUARE_NB;
}
...
constexpr Direction get_direction(Square dest, Square src) {
  ...
  if (file_diff == 0)
    return (rank_diff > 0) ? NORTH : SOUTH;
  if (rank_diff == 0)
    return (file_diff > 0) ? EAST : WEST;
  if (file_diff == rank_diff)
    return (file_diff > 0) ? NORTH_EAST : SOUTH_WEST;
  if (file_diff == -rank_diff)
    return (file_diff > 0) ? SOUTH_EAST : NORTH_WEST;
...
constexpr Value PieceValue[] = {PAWN_VALUE, KNIGHT_VALUE, BISHOP_VALUE,
...
constexpr Piece piece_of(Color c, PieceType pt) {
