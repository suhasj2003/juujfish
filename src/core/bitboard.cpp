...
namespace Juujfish {
    ...
    void BitBoards::init() {
        ...
        for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1) {
            ...
            for (int step : {-17, -15, -10, -6, 6, 10, 15, 17}) {
                if (is_square(Square(s1 + step)) && manhattan_distance(s1, Square(s1 + step)) <= 3)
                    PseudoAttacks[KNIGHT][s1] |= square_to_bb(Square(s1 + step));
            }

            PseudoAttacks[QUEEN][s1] = PseudoAttacks[BISHOP][s1] = attacks_bb(s1, BISHOP, 0);
            PseudoAttacks[QUEEN][s1] |= PseudoAttacks[ROOK][s1] = attacks_bb(s1, ROOK, 0);

            for (Direction d : {NORTH, EAST, WEST, SOUTH, NORTH_EAST, NORTH_WEST, SOUTH_EAST, SOUTH_WEST}) {
    ...
    Magic find_magic(Square s, PieceType pt, int sliding_attacks_index) {
        ...
        while (true) {
            ...
            if (try_magic(magic, blocker_bitboards, attacks_vec)) break;
        }
        return magic;
    }
    ...
    BitBoard generate_movement_mask(Square s, PieceType pt) {
        ...
        if (pt == BISHOP) {
            ...
            File f = file_of(s);
            Rank r = rank_of(s);
            movement_mask &= ~(1ULL << f) & ~(1ULL << (56 + f)) & ~(1ULL << 8*r) & ~(1ULL << (7 + 8*r));
        }
